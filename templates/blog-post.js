...
const BlogPostTemplate = ({ post, pageContext }) => {
    return (
        <Layout title={post.title} description={post.excerpt} additionalClass={['bg-white px-6']}>
            <article className="max-w-7xl mx-auto mt-20 pb-4 rounded-2xl bg-light-gray overflow-hidden">
                <Head>
                    <meta property="og:image" content={post.headerImage[0]?.localFile} />
                    <title>{config.siteMetadata.title} | {post.title}</title>
                </Head>
