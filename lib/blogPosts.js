...
export async function getPreviousBlogPost(date) {
    const url = `${apiUrl}/api/v1/content/blogpost?page=${1}&limit=${1}&order_by=internal.createdAt&order_direction=asc&hydrate=0&auth_token=${apiKey}&filters={"internal.createdAt":{"type":"lessThan", "filter": "${date}"}}`;
    const res = await fetch(
        url
    )
    return res.json()
...
export async function getNextBlogPost(date) {
    const url = `${apiUrl}/api/v1/content/blogpost?page=${1}&limit=${1}&order_by=internal.createdAt&order_direction=asc&hydrate=0&auth_token=${apiKey}&filters={"internal.createdAt":{"type":"greaterThan","filter":"${date}"}}`;
    const res = await fetch(
        url
    )
    return res.json()
